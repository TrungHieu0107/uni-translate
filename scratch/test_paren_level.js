  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (isSection) {
      ...
      if (type === "func") {
        ...
        // Parentheses (Close) - same level as the Open one because nestStack was popped
        result += "\n" + "\t".repeat(nestStack.length + sectionIndent) + ")";
      }
    } else if (isBreaker) {
      result += " " + upperToken;
...
console.log("--- PAREN LEVEL TEST ---");
const sql = "INSERT INTO WK_DT_TOKUBAI_GENKA(TENPO_CD , SYOHIN_CD) VALUES ( 'A' , 'B' )";
